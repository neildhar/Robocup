...
CMPS10::CMPS10(Stream * _bus, byte Addr): i2cBus(_bus) {
  I2C_Address = Addr >> 1;
}

CMPS10::CMPS10(Stream * _bus, byte Addr, int _xOffset, int _xScale, int _yOffset, int _yScale): i2cBus(_bus), xOffset(_xOffset), xScale(_xScale), yOffset(_yOffset), yScale(_yScale) {
  I2C_Address = Addr >> 1;
}
...
int CMPS10::magRead(){
  if(millis()-lastMagReadTime>15){
    ...
    i2cBus->write(byte(0x21));
    while(i2cBus->available()<6) delayMicroseconds(1);    
    //i2cBus->endTransmission();
    
    //i2cBus->requestFrom(I2C_Address, 4);
    //while(Serial1.available()<6)  delayMicroseconds(1);
